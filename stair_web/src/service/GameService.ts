 class GameService {
    ...
    public getBalance(username,token,onComplete,onIOError,onProgress){
        let paramJson = {
    ...
    public bet(username:string,betType: string , singlebet: string , totalbet: string , gamecode: string ,token, onComplete, onIOError, onProgress) {
    ...
    private static stringToHex(s: string) {
    let r = "";
...
private static des(key: string, message: string, encrypt: boolean, mode: number, iv: string, padding: number) {
    ...
    const keys = GameService.des_createKeys(key);
    let m: number = 0;
    let i: number, j: number, temp: number, right1: number, right2: number, left: number, right: number,
        looping: number[];
    let cbcleft: number|undefined, cbcleft2: number|undefined, cbcright: number|undefined, cbcright2: number|undefined;
    let endloop: number, loopinc: number;
    ...
    while (m < len) {
        ...
        if (mode == 1) {
            if (encrypt) {
                if (cbcleft != undefined && cbcright != undefined) {
                    left ^= cbcleft;
                    right ^= cbcright;
                }
            } else {
        ...
        if (mode == 1) {
            if (encrypt) {
                ...
                if (cbcleft2 != undefined && cbcright2 != undefined) {
                    left ^= cbcleft2;
                    right ^= cbcright2;
                }
            }
...
private static des_createKeys(key: string) {
    //declaring this locally speeds things up a bit
    ...
    //other variables
    let lefttemp:number, righttemp:number,temp:number;
        let m = 0, n = 0;
